...
One of the strengths of NOMAD is that it allows you to search entries based on rich metadata. There are different types of metadata: some are extracted automatically by NOMAD upon uploading the data (e.g., upload ID, date, author), others are generated by NOMAD parsers, and some are provided directly by the uploader. Once data and metadata are connected through schemas in NOMAD, it becomes easy to search within these datasets using NOMAD's powerful search filters.
...
Once (meta)data are stored using schemas in NOMAD they become machine actionable. NOMAD has pre-prepared several customizable widgets that allow users to create advanced dashboards for exploring data. The buttons to create customizable widgets are located just below the search bar in the Explore menu.
...
# How to Create Customizable Widgets


Let's provide an example to understand how to use these widgets better.

## Searching for ETL Materials in Solar Cells 

Imagine we are working on solar cell research. We fabricated solar cell devices with the absorber material 'CsPbBrI' (Cesium Lead Bromine Iodide), a mixed halide perovskite. The device's structure is illustrated below:
...
1. **Start with the Periodic Table**:
    - Click on the PERIODIC TABLE widget and use the (+) button to pin it to the dashboard.
...
2. **Use the TERMS Widget**:
    - To find out what ETL and HTL materials are used in the available data, use the TERMS widget.
...
3. **Create a Scatter Plot**:
    - Use the SCATTER PLOT widget to visualize the relationship between Voc, short circuit current density, and efficiency.
